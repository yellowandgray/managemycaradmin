...
import { SortColumn, SortDirection } from './subscriptions-sortable.directive';

interface SearchResult {
  countries: SubscriptionModel[];
  total: number;
}
...
function sort(countries: SubscriptionModel[], column: SortColumn, direction: string): SubscriptionModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: SubscriptionModel, term: string, pipe: PipeTransform) {
  return country.type.toLowerCase().includes(term.toLowerCase())
    ...
    || country.action.toLowerCase().includes(term.toLowerCase())
    ;
}
...
export class SubscriptionService {
  ...
  private _countries$ = new BehaviorSubject<SubscriptionModel[]>([]);
