...
function sort(countries: ListModel[], column: SortColumn, direction: SortDirection): ListModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: ListModel, term: string, pipe: PipeTransform) {
  return country.invoice_no.toLowerCase().includes(term.toLowerCase())
    ...
    || country.invoice_amount.toLowerCase().includes(term.toLowerCase())
