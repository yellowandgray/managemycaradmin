...
import { SortColumn, SortDirection } from '../products/products-sortable.directive';

interface SearchResult {
  countries: productgridModel[];
  total: number;
}
...
function sort(countries: productgridModel[], column: SortColumn, direction: string): productgridModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: productgridModel, term: string, pipe: PipeTransform) {
  return country.title.toLowerCase().includes(term.toLowerCase())
    || country.stock.toLowerCase().includes(term.toLowerCase())
    || country.category.toLowerCase().includes(term.toLowerCase())
    || country.price.toLowerCase().includes(term.toLowerCase())
    || country.discount.toLowerCase().includes(term.toLowerCase())
    || country.orders.toLowerCase().includes(term.toLowerCase())
    || country.ratings.toLowerCase().includes(term.toLowerCase())
    || country.publish.toLowerCase().includes(term.toLowerCase())
    ;
}
...
export class ProductGridService {
  ...
  private _countries$ = new BehaviorSubject<productgridModel[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 5. Rate Filter       
    if (this.productRate) {
      countries = countries.filter(country => country.ratings >= this.productRate);
    }
    else {
      countries = countries;
    }
