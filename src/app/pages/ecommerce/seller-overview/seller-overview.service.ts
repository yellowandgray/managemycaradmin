...
import { SortColumn, SortDirection } from './seller-overview-sortable.directive';

interface SearchResult {
  countries: sellerModel[];
  total: number;
}
...
function sort(countries: sellerModel[], column: SortColumn, direction: string): sellerModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: sellerModel, term: string, pipe: PipeTransform) {
  return country.name.toLowerCase().includes(term.toLowerCase())
    ...
    || country.publishedDate.toLowerCase().includes(term.toLowerCase())
    ;
}
...
export class SellerOverviewService {
  ...
  private _countries$ = new BehaviorSubject<sellerModel[]>([]);
