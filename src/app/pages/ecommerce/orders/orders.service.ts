...
function sort(countries: OrderModel[], column: SortColumn, direction: string): OrderModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: OrderModel, term: string, pipe: PipeTransform) {
  return country.order_date.toLowerCase().includes(term.toLowerCase())
    || country.delivery_date.toLowerCase().includes(term.toLowerCase())
    || country.product.toLowerCase().includes(term.toLowerCase())
    || country.customer.toLowerCase().includes(term.toLowerCase())
    || country.pay_method.toLowerCase().includes(term.toLowerCase())
    || country.price.toLowerCase().includes(term.toLowerCase())
    || country.ratings.toLowerCase().includes(term.toLowerCase())
    || country.delivery_status.toLowerCase().includes(term.toLowerCase())
    ;
}
...
export class OrderService {
  ...
  private _countries$ = new BehaviorSubject<OrderModel[]>([]);
  ...
  constructor(private pipe: DecimalPipe) {
    ...
    this.products = orderList.reverse();
  }
