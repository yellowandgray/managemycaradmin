...
function sort(countries: TicketModel[], column: SortColumn, direction: string): TicketModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: TicketModel, term: string, pipe: PipeTransform) {
  return country.ticketTitle.toLowerCase().includes(term.toLowerCase())
    || country.clientName.toLowerCase().includes(term.toLowerCase())
    || country.createDate.toLowerCase().includes(term.toLowerCase())
    || country.dueDate.toLowerCase().includes(term.toLowerCase())
...
export class TicketListService {
  ...
  private _countries$ = new BehaviorSubject<TicketModel[]>([]);
  ...
  constructor(private pipe: DecimalPipe) {
    ...
    this.products = ticketList.reverse();
  }
