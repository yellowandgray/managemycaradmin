...
import { SortColumn, SortDirection } from './earnings-sortable.directive';

interface SearchResult {
  countries: EarningModel[];
  total: number;
}
...
function sort(countries: EarningModel[], column: SortColumn, direction: string): EarningModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: EarningModel, term: string, pipe: PipeTransform) {
  return country.transactionID.toLowerCase().includes(term.toLowerCase())
    || country.date.toLowerCase().includes(term.toLowerCase())
    || country.timestamp.toLowerCase().includes(term.toLowerCase())
...
export class EarningService {
  ...
  private _countries$ = new BehaviorSubject<EarningModel[]>([]);
