...
function sort(countries: AgencieslistModel[], column: SortColumn, direction: string): AgencieslistModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: AgencieslistModel, term: string, pipe: PipeTransform) {
  return country.since.toLowerCase().includes(term.toLowerCase())
    || country.name.toLowerCase().includes(term.toLowerCase())
    || country.property.toLowerCase().includes(term.toLowerCase())
    ...
    || country.employee.toLowerCase().includes(term.toLowerCase())
    ;
}

@Injectable({ providedIn: 'root' })
export class AgenciesListService {
  private _loading$ = new BehaviorSubject<boolean>(true);
  private _search$ = new Subject<void>();
  private _countries$ = new BehaviorSubject<AgencieslistModel[]>([]);
  ...
  constructor(private pipe: DecimalPipe) {
    ...
    this.products = agencies.reverse();
  }
