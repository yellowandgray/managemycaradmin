...
function sort(countries: AgentlistModel[], column: SortColumn, direction: string): AgentlistModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: AgentlistModel, term: string, pipe: PipeTransform) {
  return country.joiningdate.toLowerCase().includes(term.toLowerCase())
...
export class AgentListService {
  ...
  private _countries$ = new BehaviorSubject<AgentlistModel[]>([]);
  ...
  constructor(private pipe: DecimalPipe) {
    ...
    this.products = agentlistdata.reverse();
  }
