...
import { NgbdAnalyticsSortableHeader, SortEvent, SortEvent1 } from './analytics-sortable.directive';
...
@Component({
  ...
  providers: [AnalyticService, DecimalPipe]
})
...
export class AnalyticsComponent {
  ...
  toppageList!: Observable<TopPageModel[]>;
  num: number = 0;

  @ViewChildren(NgbdAnalyticsSortableHeader) headers!: QueryList<NgbdAnalyticsSortableHeader>;
  sortValue: any = 'Order Date';

  constructor(private service: AnalyticService) {
    this.browserList = service.countries$;
    this.toppageList = service.toppageDatas$;
  }
  ...
  ngOnInit(): void {
    ...
    setTimeout(() => {
      ...
      this.toppageList.subscribe(x => {
        this.pagesData = Object.assign([], x);
      });
    ...
    for (const dataItem of this.statData) {
...
*/
  ...
  sortBy({ column, direction }: SortEvent, value: any) {
  ...
  topSort({ column1, direction }: SortEvent1) {
    // resetting other headers
    this.headers.forEach(header => {
      if (header.sortable1 !== column1) {
        header.direction = '';
      }
    });

    this.service.sortColumn1 = column1;
    this.service.sortDirection = direction;
  }
