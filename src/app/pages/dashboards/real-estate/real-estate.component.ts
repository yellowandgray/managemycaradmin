...
import { feedback, saleproperty, rentproperty } from './data';
import { Observable } from 'rxjs';
import { RealEstateService } from './real-estate.service';
import { DecimalPipe } from '@angular/common';
import { NgbdRealEstateSortableHeader, propertySortEvent } from './real-estate-sortable.directive';
...
@Component({
  ...
  providers: [RealEstateService, DecimalPipe]
})
...
export class RealEstateComponent {
  ...
  // Table data
  propertyList!: Observable<PropertyModel[]>;

  sortValue: any = 'Property Name'

  @ViewChildren(NgbdRealEstateSortableHeader) headers!: QueryList<NgbdRealEstateSortableHeader>;

  constructor(public service: RealEstateService) {
    this.propertyList = service.countries$;
  ...
  ngOnInit(): void {
    ...
    // Fetch Data
    setTimeout(() => {
      this.propertyList.subscribe(x => {
        this.propertyData = Object.assign([], x);
      });
...
*/
  ...
  // Sort Data
  sortBy({ column, direction }: propertySortEvent,value:any) {
    this.sortValue = value;
    this.onSort({ column, direction })
  }

  onSort({ column, direction }: propertySortEvent) {
    // resetting other headers
    this.headers.forEach(header => {
      if (header.propertysortable !== column) {
        header.direction = '';
      }
    });
